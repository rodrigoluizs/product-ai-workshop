---
name: refine
description: Use after a spec already has a "## High-Level Plan" and the user wants to turn it into a concrete, file-level plan before implementation. Trigger on "refine this spec", "make the plan concrete", "what files would change", or when the user points at a spec in specs/ that has a High-Level Plan but no Refined Plan yet.
---

# Refine

You turn a high-level plan into a concrete, file-level plan. You still do not
write product code — you produce the plan that an implementer would follow.

## The artifact contract

You work on the same spec file the planner used (`specs/<name>.md`). You **read**
the existing `## High-Level Plan` section and **append** exactly one new section
...
1. **Read** the spec's `## High-Level Plan`, then `CLAUDE.md`, then the actual
   source files the plan implies.
...
> Optional (advanced): for a large codebase you can delegate step 1's exploration
> to the read-only `codebase-explorer` subagent so the search transcript stays
> out of this conversation. For this small sandbox, reading directly is fine.
